    ...
    <body>
        ...
        <div class="container pt-5">
            <form action="/noticias/{{ $noticia->id }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="titulo">Título</label>
                    <input type="text" name="titulo" id="titulo" placeholder="Digite o título da notícia" class="form-control" value="{{ $noticia->titulo }}">
                </div>
            
                <div class="form-group">
                    <label for="conteudo">Conteúdo</label>
                    <textarea name="conteudo" id="conteudo" placeholder="Digite o conteúdo da notícia" class="form-control" rows="5">{{ $noticia->conteudo }}</textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    @if ( $noticia->imagem )
                        <img src="{{ $noticia->imagem }}" alt="erro" height="100px" class='d-block'>                    
                    @endif
                </div>    
                ...
                @foreach ( $noticia->comentarios as $comentario )
                    <div>
                        ...
                        <p class="text-muted"> Notícia criada em: {{ $comentario->noticia->data_publicacao->format("d/m/Y") }}</p>
                    </div>    
                @endforeach
